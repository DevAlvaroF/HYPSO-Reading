...
class Chlorophyll:
    ...
    def interpolate_spectra_lon_lat(self, sat_name, hypercube_to_interpolate):
        ...
        max_col = coord_condition.shape[1] - np.min(np.ma.masked_equal(last_on_row, 0.0, copy=False))
        ...
        max_row = coord_condition.shape[0] - np.min(np.ma.masked_equal(last_on_col, 0.0, copy=False))

        other_sat_lat_cropped = other_sat_lat[min_row:max_row + 1, min_col:max_col + 1]
        other_sat_lon_cropped = other_sat_lon[min_row:max_row + 1, min_col:max_col + 1]
        ...
        for i in tqdm(range(hypercube_to_interpolate.shape[2])):
            ...
            # Experimental
            chanel = hypercube_to_interpolate[min_row:max_row + 1, min_col:max_col + 1, i]
    ...
    def ocx_estimation(self, satName=None, interpMethodUsed=None, special_extent=None,
        ...
        satellite_wl, hypercube, spatial_dim, water_mask = self.get_data_for_sat(satName, interpMethodUsed)
        # if satName == 'hypso':
        ...
        if ('hypso' not in satName) and (interpMethodUsed is not None):
            ...
            hypercube_to_save = self.interpolate_spectra_lon_lat(satName, hypercube_to_save)
        ...
        with np.errstate(divide='ignore'):
            ...
            lmbr = np.where(np.logical_or(mbr2 == 0, mbr2 == np.nan), np.nan, np.log10(mbr2))
            ...
            if satName == 'hypso':
                ...
                if interpMethodUsed is not None:
                    chl_external_key = interpMethodUsed + '_' + satName + '_' + chl_key  # 'interp_aqua for example
